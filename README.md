...
- 去中心化解决方案。
- 支持上百台不同业务的应用服务器接入。任务在每个应用上独立执行，充分利用现有应用的计算资源
- 故障转移。应用如有多台执行机器，一台机器挂掉，其上的任务会切到其它机器。
- 控制台统一管理任务修改、启动、停止等。控制台挂掉不影响任务的执行。
- 可以在线查看任务日志，实时了解任务执行情况。


![架构图](https://raw.githubusercontent.com/cehome-com/easy-task/master/images/system.png)
...
- 执行命令： java -jar task-console.jar
...
![demo图](https://raw.githubusercontent.com/cehome-com/easy-task/master/images/main.png)

控制台缺省内置了一个H2数据库（端口9092）来保存任务配置。

如果你要修改demoPlugin，可以剪出task-console-core模块 ，这是一个spring boot应用，修改类com.cehome.task.console.DemoPlugin后，直接启动com.cehome.task.console.TaskConsoleApplication。

## 模拟应用（worker）接入调度平台
实际使用中，console只是管理任务，不执行任务，任务是在别的应用中执行的。接着上面，模拟app1和app2两个应用接入调度平台。为了方便，还是用task-console.jar来模拟。执行前，先保证上面的console还在运行状态。
...
- 访问http://localhost:8080 ，点击demo的“修改”按钮，弹出修改界面，点击“应用”下拉框，应该能看到app1、app2也在里面，选择app2，然后保存并关闭。
...
![添加任务](https://github.com/cehome-com/easy-task/raw/master/images/addTask.png)
